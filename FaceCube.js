function FaceCube(cubeString) {
	this.f = typeof subeString !== 'undefined' ? map.call(cubeString, (c)=>Color[c]) : [Color.U, Color.U, Color.U, Color.U, Color.U, Color.U, Color.U, Color.U, Color.U, 
	...
	this.toCubieCube = function() {
		...
		for (var i in Corner) {
			...
			for (ori = 0; ori < 3; ori++) {
				if (this.f[this.cornerFacelet[i][ori]] == Color.U || this.f[this.cornerFacelet[i][ori]] == Color.D) {
					break;
				}
			}
			col1 = this.f[this.cornerFacelet[i][(ori + 1) % 3]];
			col2 = this.f[this.cornerFacelet[i][(ori + 2) % 3]];

			for (var j in Corner) {
				j = Corner[j];
				if (col1 == this.cornerColor[j][1] && col2 == this.cornerColor[j][2]) {
		...
		for (var i in Edge) {
			...
			for (var j in Edge) {
				j = Edge[j];
				if (this.f[this.edgeFacelet[i][0]] == this.edgeColor[j][0] 
					&& this.f[this.edgeFacelet[i][1]] == this.edgeColor[j][1]) {
					cubieCube.edgePermutation[i] = j;
					cubieCube.edgeOrientation[i] = 0;
					break;
				}
				if (this.f[this.edgeFacelet[i][0]] == this.edgeColor[j][1]
						&& this.f[this.edgeFacelet[i][1]] == this.edgeColor[j][0]) {
					cubieCube.edgePermutation[i] = j;
