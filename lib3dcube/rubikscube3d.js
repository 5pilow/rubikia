 ...
 */
RubiksCube3D=function(colors) {
  this.RubiksCube3DInit(colors);
}
...
RubiksCube3D.prototype.RubiksCube3DInit=function(colors) {
  this.cornerCount=8;
  this.edgeCount=12;
  this.sideCount=6;
  this.centerCount=1;
  ...
  this.attributes=this.createAttributes(colors);
...
RubiksCube3D.prototype.createAttributes=function(colors) {
  ...
  for (var i=0;i<6;i++) {
	...
	for (var i = 0; i < 54; i++) {
		if (colors[i] == 6) { // no sticker
			a.stickersVisible[i] = false; 
		} else {
			a.stickersVisible[i] = true; 
		    a.stickersFillColor[i] = faceColors[colors[i]];
		}
	}
